...
class TestAnalysisMethods(unittest.TestCase):
    ...
    def test_ingest_marker_regex(self):
        ...
        adata = ingest_cells(df1, "marker*", obs = batch)
    ...
    def test_ingest_marker_list(self):
        ...
        adata = ingest_cells(df1, ["marker1", "marker2"], obs = batch)
    ...
    def test_ingest_marker_name(self):
        ...
        adata = ingest_cells(df1, "marker1", obs = batch)
        self.assertCountEqual(
            list(adata.var_names), 
            ["marker1"])
    ...
    def test_ingest_region_list(self):
        ...
        adata = ingest_cells(df1, "marker1", obs = ["region1", "region2"])
        self.assertCountEqual(
            list(adata.obs_keys()), 
            ["region1", "region2"])
