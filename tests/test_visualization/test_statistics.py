...
from spac.utils import compute_pairwise_stats_multi
...
class TestComputePairwiseStatsMulti(unittest.TestCase):
    ...
    def test_wilcoxon_with_unequal_sample_sizes(self):
        ...
        df = pd.DataFrame(
            {"group": ["A"] * 3 + ["B"] * 5, "feature1": [1, 2, 3, 4, 5, 6, 7, 8]}
        )
        result = compute_pairwise_stats_multi(
            df=df,
            group_col="group",
    ...
    def test_wilcoxon_equal_sample_sizes(self):
        ...
        df = pd.DataFrame(
            {"group": ["A"] * 4 + ["B"] * 4, "feature1": [1, 2, 3, 4, 5, 6, 7, 8]}
        )
        result = compute_pairwise_stats_multi(
            df=df,
            group_col="group",
    ...
    def test_significance_annotation(self):
        ...
        df = pd.DataFrame(
            {"group": ["A"] * 10 + ["B"] * 10, "feature1": [1] * 10 + [100] * 10}
        )
        result = compute_pairwise_stats_multi(
            df=df,
            group_col="group",
    ...
    def test_nan_in_features(self):
        ...
        df = pd.DataFrame(
            {"group": ["A", "A", "B", "B"], "feature1": [1, np.nan, 2, 3]}
        )
        result = compute_pairwise_stats_multi(
            df=df,
            group_col="group",
