...
class TestPrepareSpatialDistanceData(unittest.TestCase):
    ...
    def setUp(self):
        # Create a minimal deterministic AnnData object for testing
        ...
        # Distances:
        # Cell1 to A=0.0, B=1.0
        # Cell2 to A=1.0, B=0.0
        # Cell3 to A=0.5, B=0.5
    ...
    def _convert_expected_to_categorical(self, df, stratify_by=None):
        """Helper to convert columns to categorical for comparison."""
    ...
    def test_io_correctness_minimal(self):
        """
        Test a typical scenario with minimal input, no distance_to or
        stratify_by. Ensures that output DataFrame is tidy and correct.
        """
        ...
        # After filtering (distance_from='A'), keep Cell1, Cell3.
        # Distances:
        # Cell1: A=0.0, B=1.0
        # Cell3: A=0.5, B=0.5
        # Tidy long form:
        # cellid | group | dist | pheno
        # Cell1 | A | 0.0 | A
        # Cell1 | B | 1.0 | A
        # Cell3 | A | 0.5 | A
        # Cell3 | B | 0.5 | A
        ...
        df_res_srt = df_result.sort_values(
            ['cellid', 'group']
        ).reset_index(drop=True)
        exp_srt = expected.sort_values(
            ['cellid', 'group']
        ).reset_index(drop=True)
    ...
    def test_with_distance_to_subset(self):
        """
        Test using distance_to as a subset of the available categories.
        Original categories: A, B
        Choose distance_to = ['B'] only.
        This filters out 'A' rows before reordering categories.
        """
        df_result = _prepare_spatial_distance_data(
            adata=self.adata,
            annotation='cell_type',
        ...
        self.assertListEqual(list(df_res_srt['group'].cat.categories), ['B'])
    ...
    def test_log_transform(self):
        ...
        # Cell1: A=0.0, B=1.0 -> log1p: A=0.0, B=~0.693147
        # Cell3: A=0.5, B=0.5 -> log1p(0.5)=~0.405465
    ...
    def test_error_missing_phenotypes_in_distance_map(self):
        ...
        distance_map columns. Re-setup self.adata to include a phenotype 'C'
        in obs but not in distance_map columns.
        """
        data = np.array([[1.0], [2.0], [3.0]])
