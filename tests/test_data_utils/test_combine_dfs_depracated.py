...
import unittest
import pandas as pd
from spac.data_utils import combine_dfs_depracated
...
class TestCombineDFs(unittest.TestCase):
    ...
    def test_combine_dfs(self):
        combined_df = combine_dfs_depracated(self.dataframes, self.annotations)
    ...
    def test_combine_dfs_wrong_annotations_type(self):
        with self.assertRaises(TypeError):
            combine_dfs_depracated(self.dataframes, "wrong_type")

    def test_combine_dfs_missing_annotation(self):
        annotations_missing_data = self.annotations.drop("file1.csv")
        with self.assertRaises(ValueError):
            combine_dfs_depracated(self.dataframes, annotations_missing_data)
