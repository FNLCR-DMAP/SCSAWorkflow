...
Refactored to use generalized save_results with blueprint outputs configuration.
...
def run_from_json(
    ...
    show_plot: bool = True,
    outputs_config: Optional[Dict[str, str]] = None
) -> Union[Dict[str, List[str]], Tuple[Any, pd.DataFrame]]:
    """
    Execute Boxplot analysis with parameters from JSON.
    Replicates the NIDAP template functionality with generalized output handling.
    ...
    outputs_config : dict, optional
        Output configuration from blueprint JSON. If None, uses default:
        {"DataFrames": "dataframe_folder", "figures": "figure_folder"}

    Returns
    -------
    dict or tuple
        If save_results_flag=True: Dictionary of saved file paths
        If save_results_flag=False: Tuple of (figure, summary_dataframe)
    """
    # Parse parameters from JSON
    params = parse_params(json_path)

    # Get outputs configuration - from params or use default
    if outputs_config is None:
        # Try to get from params (if blueprint JSON was passed)
    ...
    if save_results_flag:
        # Prepare results dictionary organized by type
        results = {}
        
        # Handle figures
        if "figures" in outputs_config or "figure_folder" in outputs_config.values():
            results["figures"] = {"boxplot": fig}
            
            # If there are multiple features, could generate multiple figures
            # (extend this logic as needed for your specific requirements)
        
        # Handle DataFrames
        if "DataFrames" in outputs_config or "dataframe_folder" in outputs_config.values():
            results["dataframes"] = {
                "summary_statistics": summary_df,
                # Could add raw data frame if needed
                "plot_data": df
            }
        
        # Get output base directory from params or use current directory
        ...
        logging.info(f"Boxplot completed. Files saved:")
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        print(
            "Usage: python boxplot_template.py <params.json> [blueprint.json]",
            file=sys.stderr
        )
        print(
        ...
        print(
            "  blueprint.json: Optional blueprint file with outputs config",
            file=sys.stderr
        )
        sys.exit(1)
    ...
    # Load outputs config from blueprint if provided
    outputs_config = None
    if len(sys.argv) > 2:
    ...
    result = run_from_json(sys.argv[1], outputs_config=outputs_config)
